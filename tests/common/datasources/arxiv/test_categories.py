...
async def test_arxiv_fetch_subjects(httpx_async_client):
    ...
    """
    fetcher = ArxivCategoryFetcher(client=httpx_async_client)
    counter = 0
...
def test_arxiv_parse_domain():
    ...
    """
    domains = {}
    ArxivCategoryFetcher._parse_set("cs", "Computer Science", domains)

    assert (
        len(domains) == 1 and domains["cs"].code == "cs"
    ), "Expected domain code to be 'cs'"

    subject = ArxivCategoryFetcher._parse_set(
        "cs:cs:ai", "Artificial Intelligence", domains
    )
